...
The repository includes a folder called `Sorted_pathways` which contains all the files defining the pathway groups (GROUPS) used by the functions. Additionally, two `.rds` files are also provided. These objects can be regenerated by running the corresponding steps in the R Markdown file; however, these steps can be skipped if preferred, and the saved `.rds` files can be used directly.
